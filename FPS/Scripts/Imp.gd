...
onready var Imp = $Eyes #I then reference a spatial node that represents the eyes, it is used so that the grunt will look where the 
#eyes are looking
...
func _ready(): #everything in here will occur as soon as the node thats attatched to this script starts
	...
	impHealth = maxHealth #on ready the gruntHealth is reset to the maxHealth
